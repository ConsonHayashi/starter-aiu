...
export class LoginPage implements OnInit {
  ...
  login() { 
    if (this.loginForm.valid) {
      this.loginService.signIn(this.loginForm.value).subscribe((data: Message<any>) => this.loginCallback(data));
    }
  }

  loginCallback(msg: Message<any>) {
      console.log(msg);
      if( msg.status == 504 ){
        this.commonUI.log("账号或密码输入错误");
      }else if (msg.status == 200) {
        this.commonUI.log("登陆成功");
       
        const jwt = {auth: msg.extra.jwt, name: this.loginForm.get('username').value, id: msg.extra.id}
        this.global.setJwt(jwt);
        // console.log(this.global.getUserNoPolicy())
        console.log(this.global.getJwt());
